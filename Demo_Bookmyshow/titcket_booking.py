            ...
            if (len(jsonPopup)):
                for item in jsonPopup:
                    if (float(item["price"]) <= 150 and item["availabilityText"].lower() == "available"):
                        avaialbleFinalList.append({"Name": theatre.attrs['data-name'],
                                                   "Time": time.contents[0].attrs['data-display-showtime']})
...
def getAvailableShowsAndEmail(findMovie, searchFrom, searchFor, bookmyshow_base, goHref, toEmail, prefferedTheatres=[]):
    ...
    for day in listOfDatesRequired:
        ...
        msg1 = "******************** List of  Shows for  " + dayInFormat + " - " + getDayName(
            dayNumber) + " " "****************\n"
...
def findTickets(argv):
    ...
    foundTickets = getAvailableShowsAndEmail(findMovie, searchFrom, searchFor, bookmyshow_base, goHref, toEmail,
                                             prefferedTheatres)
    return foundTickets
